...
class CheckoutController extends GetxController {
  ...
  // Steps
  final List<String> steps = ['Shopping Bag', 'Shipping', 'Payment', 'Summary'];
    ...
    try {
      ...
      if (savedAddresses.isNotEmpty) {
        final defaultAddress =
            savedAddresses.firstWhereOrNull((addr) => addr.isDefault);
  ...
  Future<void> placeOrder() async {
    ...
    try {
      ...
      for (final item in cartItems) {
        ...
        final bool hasInventoryByVariant =
            productData['inventoryByVariant'] == true;
        ...
        if (hasInventoryByVariant) {
          ...
          if (variantQuery.docs.isEmpty) {
            CustomToast.error(
                'Selected variant of ${item.name} is no longer available');
            isProcessing.value = false;
            return;
          }
          ...
          if (currentStock < item.quantity) {
            CustomToast.error(
                'Not enough stock for ${item.name} (${item.size}, ${item.color})');
            isProcessing.value = false;
            return;
          }

          // Update variant stock
          stockUpdateBatch.update(variantDoc.reference,
          ...
          stockUpdateBatch.update(productDoc.reference,
              {'stock': FieldValue.increment(-item.quantity)});
        }
      ...
      CustomToast.error('Failed to place order');
    } finally {
      isProcessing.value = false;
    }
  }
  ...
  Future<void> _updateDefaultAddressStatus(
      String userId, String exceptAddressId) async {
    final snapshot = await _firestore
        .collection('users')
        .doc(userId)
        .collection('addresses')
